...
def monos(activos_close,activos_volumen,n_activos,capital_inicial,com, activos_open):
    serie_momentum = []
    serie_EW = []
    serie_reb = []
    serie_vol = []
    ...
    for i in range(22,len(activos_close)):
        if activos_close.index[i].month != activos_close.index[i-1].month:
            reb_monos = pd.DataFrame(np.random.dirichlet(np.ones(4), size=1))
            if i==22:
    ...
    return serie_reb, comision_total, diferencias
