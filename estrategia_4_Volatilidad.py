...
def obtener_precios_vol(activos_close,n_activos,capital_inicial,com, dias_reb, etiquetas):
    ...
    for i in range(22,len(activos_close)):# for que va leyendo el dataframe de datos
        if i==22 or (etiquetas.iloc[i,0] != etiquetas.iloc[i-1,0]):# si hay cambio de mes se rebalancea
            ...
            rent_activos = np.log(activos_close).diff()[i-40:i-1].dropna(axis=0)# se calcula la rentabilidad de los activos en los ultimos 20 dias
